 ...
 * The generic CommentModel object should be materialized to CommentPark object
 */
@Component("commentParkDao")
public class CommentParkDaoImpl implements CommentDao{
	...
	public boolean delete(int id) {
		CommentPark comment = new CommentPark();
		comment.setId(id);
		ht.delete(comment);
		return true;
	}
	...
	public List<CommentModel> getAll() {
		return (List<CommentModel>) getAllParkComments();
	}
	...
	public Collection<CommentModel> getAllParkComments(){
		return this.ht.find("from CommentPark");
	}
	...
	public Collection<CommentModel> getManyByDateDiff(Date from, Date to){
		return this.ht.find("from CommentPark c where c.commentDate > ? and c.commentDate < ?", from, to);
	}

	@Override
	public CommentModel get(int id) {
		return ht.get(CommentPark.class, id);
	}
