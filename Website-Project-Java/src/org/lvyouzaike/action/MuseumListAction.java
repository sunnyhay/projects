...
import org.lvyouzaike.service.admin.impl.CommentMuseumAdminServiceImpl;
import org.lvyouzaike.service.admin.impl.ExpressSupportAdminServiceImpl;
import org.lvyouzaike.service.impl.CommentMuseumServiceImpl;
...
@Component("museumListAction")
@Scope("prototype")
public class MuseumListAction extends ActionSupport {
	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;
	private String nickname;  //the nickname input on museum comment area
	private String comment;   //the comment input on museum comment area
	...
	// when the page is loaded, fetch the total count of support express for museum list
	public String getExpressCount() throws IOException {
		...
		String name = "museumSupport"; // name of the item in express support table for this museum list
		int total = 0;
		...
		resp.setHeader("Cache-Control", "no-cache"); // cancel cache when ajax
														// works
	...
	public String express() throws IOException{
		...
		List<Integer> museumSupports = new ArrayList<Integer>();
		ActionContext context = ActionContext.getContext();
		Map<String, Object> map = context.getApplication();
		
		String name = "museumSupport";
		...
		if(null == map.get(support)){//the first time, need to fetch support express data of museum list out of db
			...
			map.put("museum1", es.getItem1());
			map.put("museum2", es.getItem2());
			map.put("museum3", es.getItem3());
			map.put("museum4", es.getItem4());
			map.put("museum5", es.getItem5());
		}else{
		...
		museumSupports.add((Integer)map.get("museum1"));  //prepare the result to be sent back to ajax callback method
		museumSupports.add((Integer)map.get("museum2"));
		museumSupports.add((Integer)map.get("museum3"));
		museumSupports.add((Integer)map.get("museum4"));
		museumSupports.add((Integer)map.get("museum5"));
		
		Gson gson = new Gson();
		String result = gson.toJson(museumSupports);
	...
	public String getMuseumComments() throws IOException{
		ApplicationContext applicationContext = new ClassPathXmlApplicationContext("beans.xml");
		CommentAdminService commentMuseumAdminService = (CommentMuseumAdminServiceImpl) applicationContext.getBean("commentMuseumAdminService");
		List<CommentModel> comments = commentMuseumAdminService.getAll();
	...
	public String addMuseumComment() throws Exception{
		nickname = URLDecoder.decode(nickname, "UTF-8");
		comment = URLDecoder.decode(comment, "UTF-8");
				
		ApplicationContext applicationContext = new ClassPathXmlApplicationContext("beans.xml");
		CommentService commentMuseumService = (CommentMuseumServiceImpl) applicationContext.getBean("commentMuseumService");
		
		boolean add = commentMuseumService.addComment(nickname, comment);
