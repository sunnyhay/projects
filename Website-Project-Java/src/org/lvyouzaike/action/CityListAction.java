...
import org.lvyouzaike.service.admin.impl.CommentCityAdminServiceImpl;
import org.lvyouzaike.service.admin.impl.ExpressSupportAdminServiceImpl;
import org.lvyouzaike.service.impl.CommentCityServiceImpl;
...
@Component("cityListAction")
@Scope("prototype")
public class CityListAction extends ActionSupport {
	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;
	private String nickname;  //the nickname input on city comment area
	private String comment;   //the comment input on city comment area
	...
	//when the page is loaded, fetch the total count of support express for city list 
	public String getExpressCount() throws IOException{
		...
		String name = "citySupport";  //name of the item in express support table for this city list
		int total = 0;
	...
	public String express() throws IOException{
		...
		List<Integer> citySupports = new ArrayList<Integer>();
		ActionContext context = ActionContext.getContext();
		Map<String, Object> map = context.getApplication();
		
		String name = "citySupport";
		...
		if(null == map.get(support)){//the first time, need to fetch support express data of city list out of db
			...
			map.put("city1", es.getItem1());
			map.put("city2", es.getItem2());
			map.put("city3", es.getItem3());
			map.put("city4", es.getItem4());
			map.put("city5", es.getItem5());
		}else{
		...
		citySupports.add((Integer)map.get("city1"));  //prepare the result to be sent back to ajax callback method
		citySupports.add((Integer)map.get("city2"));
		citySupports.add((Integer)map.get("city3"));
		citySupports.add((Integer)map.get("city4"));
		citySupports.add((Integer)map.get("city5"));
		
		Gson gson = new Gson();
		String result = gson.toJson(citySupports);
	...
	public String getCityComments() throws IOException{
		ApplicationContext applicationContext = new ClassPathXmlApplicationContext("beans.xml");
		CommentAdminService commentCityAdminService = (CommentCityAdminServiceImpl) applicationContext.getBean("commentCityAdminService");
		List<CommentModel> comments = commentCityAdminService.getAll();
	...
	public String addCityComment() throws Exception{
		nickname = URLDecoder.decode(nickname, "UTF-8");
		comment = URLDecoder.decode(comment, "UTF-8");
				
		ApplicationContext applicationContext = new ClassPathXmlApplicationContext("beans.xml");
		CommentService commentCityService = (CommentCityServiceImpl) applicationContext.getBean("commentCityService");
		
		boolean add = commentCityService.addComment(nickname, comment);
