...
rnode preprocess(vector<string>* prefixes){
	...
	double tbmpMemUsage = 0.0;
	tbmpMemUsage += (sizeof(bitset<16384>) * 2 + sizeof(vector<tnode*>) + sizeof(rnode) + sizeof(tnode) * childNodeCounter) / (double)(1024 * 1024);
		
	cout << "tbmp lpm memory usage (MB): " << tbmpMemUsage << endl;
