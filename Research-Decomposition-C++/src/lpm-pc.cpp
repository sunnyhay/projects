...
//preprocess a classifier using LPM method of bitmap-hash
void preprocessBitmapHash(vector<string>* srcIPs, vector<string>* dstIPs, vector<string>* srcPorts,
	vector<string>* dstPorts, vector<string>* types, char* classifier, 
	...
	map<string,int>* srctable32bit, map<string,bnode>* srctable24bit, map<string,bnode>* srctable16bit, 
	map<string,bnode>* srctable8bit, bitset<256>* srcless8bit, vector<int>* srcless8hops,
	map<string,int>* dsttable32bit, map<string,bnode>* dsttable24bit, map<string,bnode>* dsttable16bit, 
	map<string,bnode>* dsttable8bit, bitset<256>* dstless8bit, vector<int>* dstless8hops,
	...
	//source IP
	lpmlib::preprocess(&srcIPsHops, srctable32bit, srctable24bit, srctable16bit, 
		srctable8bit, srcless8bit, srcless8hops);
	//destination IP
	lpmlib::preprocess(&dstIPsHops, dsttable32bit, dsttable24bit, dsttable16bit, 
		dsttable8bit, dstless8bit, dstless8hops);
	...
	//write to a file with src and dst IP records
	ofstream ipfile;
	ipfile.open ("ipstat.txt");
	...
	for(size_t i = 0; i < (*srcIPs).size(); i++){
		...
		ipfile << ruleNo;
		ipfile << " ";
		ipfile << srcIPResult.size();
		ipfile << " ";
		ipfile << dstIPResult.size();
		ipfile << "\n";
		...
		//traverse the rule hash table and sortr each rule list
...
void lookupBitmapHash(char* traceFile, map<string,int>* srcIPIDTable, map<string,int>* dstIPIDTable, map<string,emNode>* typeTable, 
	map<string,int>* srctable32bit, map<string,bnode>* srctable24bit, map<string,bnode>* srctable16bit, 
	map<string,bnode>* srctable8bit, bitset<256>* srcless8bit, vector<int>* srcless8hops,
	map<string,int>* dsttable32bit, map<string,bnode>* dsttable24bit, map<string,bnode>* dsttable16bit, 
	map<string,bnode>* dsttable8bit, bitset<256>* dstless8bit, vector<int>* dstless8hops,
	...
	if(preFile.is_open()){
		...
		while(preFile.good()){
			...
			srcResult = lpmlib::query(traceAdr, srctable32bit, srctable24bit,
					srctable16bit, srctable8bit, srcless8bit, srcless8hops);
			...
			dstResult = lpmlib::query(traceAdr, dsttable32bit, dsttable24bit,
					dsttable16bit, dsttable8bit, dstless8bit, dstless8hops);
...
int main(int argc, char* argv[])
{
	...
	map<string,int> srctable32bit;          //32-bit hash table for source IP
	map<string,bnode> srctable24bit;        //24-bit hash table for source IP
	map<string,bnode> srctable16bit;        //16-bit hash table for source IP
	map<string,bnode> srctable8bit;         //8-bit hash table for source IP
	bitset<256> srcless8bit;                //less than 8-bit bitmap for source IP
	vector<int> srcless8hops;               //less than 8-bit nexthop container for source IP

	map<string,int> dsttable32bit;          //32-bit hash table for destination IP
	map<string,bnode> dsttable24bit;        //24-bit hash table for destination IP
	map<string,bnode> dsttable16bit;        //16-bit hash table for destination IP
	map<string,bnode> dsttable8bit;         //8-bit hash table for destination IP
	bitset<256> dstless8bit;                //less than 8-bit bitmap for destination IP
	vector<int> dstless8hops;               //less than 8-bit nexthop container for destination IP

	map<unsigned int,vector<unsigned int>> ruleHTable;        //the only rule hash table

	preprocessBitmapHash(&srcIPs, &dstIPs, &srcPorts, &dstPorts, &types, classifier, 
		&srcIPPMTable, &srcIPIDTable, &dstIPPMTable, &dstIPIDTable, &typeTable,
		&srctable32bit, &srctable24bit, &srctable16bit, &srctable8bit, &srcless8bit, &srcless8hops,
		&dsttable32bit, &dsttable24bit, &dsttable16bit, &dsttable8bit, &dstless8bit, &dstless8hops,
		&ruleHTable);
	
	//test query
	char* traceFile = argv[2];
	lookupBitmapHash(traceFile, &srcIPIDTable, &dstIPIDTable, &typeTable, 
		&srctable32bit, &srctable24bit, &srctable16bit, &srctable8bit, &srcless8bit, &srcless8hops,
		&dsttable32bit, &dsttable24bit, &dsttable16bit, &dsttable8bit, &dstless8bit, &dstless8hops,
		&ruleHTable);
	...
	lpmMemUsage += (sizeof(string) + sizeof(bnode)) * srctable24bit.size() / (double)(1024 * 1024) +
		(sizeof(string) + sizeof(bnode)) * srctable16bit.size()	/ (double)(1024 * 1024) +
		(sizeof(string) + sizeof(bnode)) * srctable8bit.size()	/ (double)(1024 * 1024) +
		sizeof(bitset<256>) * srcless8bit.size() / (double)(1024 * 1024) +
		(sizeof(string) + sizeof(bnode)) * dsttable24bit.size()	/ (double)(1024 * 1024) +
		(sizeof(string) + sizeof(bnode)) * dsttable16bit.size()	/ (double)(1024 * 1024) +
		(sizeof(string) + sizeof(bnode)) * dsttable8bit.size()	/ (double)(1024 * 1024) +
		sizeof(bitset<256>) * dstless8bit.size() / (double)(1024 * 1024);
	/*cout << "size of src 24bit table (MB): " << (sizeof(string) + sizeof(bnode)) * srctable24bit.size()
		/ (double)(1024 * 1024) << endl;
	cout << "size of src 16bit table (MB): " << (sizeof(string) + sizeof(bnode)) * srctable16bit.size()
		/ (double)(1024 * 1024) << endl;
	cout << "size of src 8bit table (MB): " << (sizeof(string) + sizeof(bnode)) * srctable8bit.size()
		/ (double)(1024 * 1024) << endl;
	cout << "size of src less 8bit table (MB): " << sizeof(bitset<256>) * srcless8bit.size()
		/ (double)(1024 * 1024) << endl;
	cout << "size of dst 24bit table (MB): " << (sizeof(string) + sizeof(bnode)) * dsttable24bit.size()
		/ (double)(1024 * 1024) << endl;
	cout << "size of dst 16bit table (MB): " << (sizeof(string) + sizeof(bnode)) * dsttable16bit.size()
		/ (double)(1024 * 1024) << endl;
	cout << "size of dst 8bit table (MB): " << (sizeof(string) + sizeof(bnode)) * dsttable8bit.size()
		/ (double)(1024 * 1024) << endl;
	cout << "size of dst less 8bit table (MB): " << sizeof(bitset<256>) * dstless8bit.size()
		/ (double)(1024 * 1024) << endl;*/
