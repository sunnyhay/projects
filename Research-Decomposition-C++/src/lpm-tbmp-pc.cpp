...
//preprocess a classifier using LPM method of tbmp
void preprocessTbmp(vector<string>* srcIPs, vector<string>* dstIPs, vector<string>* srcPorts,
	vector<string>* dstPorts, vector<string>* types, char* classifier, 
	...
	rnode* srcRoot, rnode* dstRoot,	map<unsigned int,vector<unsigned int>>* ruleHTable){
	...
	//source IP
	(*srcRoot) = tbmplib::preprocess(&srcIPsHops);
	
	//destination IP
	(*dstRoot) = tbmplib::preprocess(&dstIPsHops);

	cout << "tbmp lpm processing time elasping: " << (double)(clock() - lpmStart)/CLOCKS_PER_SEC << " seconds." << endl;
...
//query the rule hash table with given input by means of LPM method of tbmp
void lookupTbmp(char* traceFile, map<string,int>* srcIPIDTable, map<string,int>* dstIPIDTable, map<string,emNode>* typeTable, 
	rnode* srcRoot, rnode* dstRoot, map<unsigned int,vector<unsigned int>>* ruleHTable){
	...
	if(preFile.is_open()){
		...
		while(preFile.good()){
			...
			srcResult = to_string(tbmplib::lookup(srcRoot, traceAdr));
			...
			dstResult = tbmplib::lookup(dstRoot, traceAdr) + "";
...
int main(int argc, char* argv[])
{
	...
	rnode srcRoot;
	rnode dstRoot; 
	preprocessTbmp(&srcIPs, &dstIPs, &srcPorts, &dstPorts, &types, classifier, 
		&srcIPPMTable, &srcIPIDTable, &dstIPPMTable, &dstIPIDTable, &typeTable,
		&srcRoot, &dstRoot, &ruleHTable);
	
	//test query
	char* traceFile = argv[2];
	lookupTbmp(traceFile, &srcIPIDTable, &dstIPIDTable, &typeTable, 
		&srcRoot, &dstRoot, &ruleHTable);
